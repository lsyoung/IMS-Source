...
const bxlcommons = {
 ...
 requestMSRData(strPrinterName, timeout, _callback) {
	var serverURL = bxlcommons.getServerURL().url;
...
checkResult(method, strPrinterName, requestId, responseId, _callback) {
	...
	var inquiryData = bxlcommons.makeResultInquiryData(requestId, responseId, 30);
	...
	xmlHttpCheck.onreadystatechange = function() {
		if (xmlHttpCheck.readyState == 4 && xmlHttpCheck.status == 200) {
			...
			if(ret.search("ready") >= 0 || ret.search("progress") >= 0)	{
				bxlcommons.checkResult(method, strPrinterName, requestId, responseId, _callback);
			}
			else {
	...
	if (connectionMode == "ws:" || connectionMode == "wss:") {
		...
		xmlHttpReq.onreadystatechange = function() {
			if (xmlHttpReq.readyState == 4 && xmlHttpReq.status == 200) {
				...
				var ret = res.Result;
				if(ret.search("ready") >= 0 || ret.search("progress") >= 0)	{
					bxlcommons.checkResult('POST', strPrinterName, res.RequestID, res.ResponseID, _callback);
				}
